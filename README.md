...
- 🎯 Entry points to execute your program using `python -m <sampleflaskprj>` or `$ sampleflaskprj` with basic CLI argument parsing.
...
---
# sampleflaskprj Flask Application

Awesome sampleflaskprj created by abeep
...
```bash
git clone https://github.com/abeep/sampleflaskprj sampleflaskprj
cd sampleflaskprj
make install
```
...
```bash
sampleflaskprj create-db   # run once
sampleflaskprj populate-db  # run once (optional)
sampleflaskprj add-user -u admin -p 1234  # ads a user
sampleflaskprj run
```
