...
original_author="abeep"
original_name="sampleflaskprj"
original_urlname="sampleflaskprj"
original_description="Awesome sampleflaskprj created by abeep" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/sampleflaskprj "${name}"
...
then
    ...
    echo "then run 'sampleflaskprj' or 'python -m sampleflaskprj'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'sampleflaskprj' or 'python -m sampleflaskprj' or 'poetry run sampleflaskprj'"
fi
